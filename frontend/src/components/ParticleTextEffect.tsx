...
const DEFAULT_WORDS = ["KUBE", "CREDENTIAL", "PLATFORM", "KUBE"]
...
export function ParticleTextEffect({ words = DEFAULT_WORDS }: ParticleTextEffectProps) {
    ...
    for (const coordIndex of coordsIndexes) {
      ...
      if (alpha > 0) {
        ...
        if (particleIndex < particles.length) {
          ...
          particle.maxForce = particle.maxSpeed * 0.03  // Slower force
          particle.particleSize = Math.random() * 6 + 6
          particle.colorBlendRate = Math.random() * 0.015 + 0.0015  // Slower color transition
  ...
  const animate = () => {
    ...
    // Auto-advance words (slower transition - every 480 frames instead of 240)
